...
matplot(x=seq(0+dt,Time,dt),y=V[,1:5],type='s',lty = 1,xlab='Time',ylab='Firm value trajectories',main='Trajectoriesof firm values in the Merton model')
r <- 0.05
K <- 80
...
dN <- matrix(val, M+1, n)
N <- apply(dN, 2, cumsum)
matplot(x=seq(0,Time,dt),y=N[,1:5],type='s',xlab='Time',ylab='Number of defaults process trajectories', main = 'Trajectories of Cox process')
...
#CreditMetrics: LossDefaultRate;Level
